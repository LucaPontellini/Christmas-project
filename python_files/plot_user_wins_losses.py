...
# Funzione per creare un grafico a torta per le vincite e perdite degli utenti
def plot_user_wins_losses(user_data):
    user_ids = list(user_data.data["users"].keys())
    wins_losses = [sum(user_data.data["users"][user_id]['wins_losses'].values()) for user_id in user_ids]
    plt.pie(wins_losses, labels=user_ids, autopct='%1.1f%%', startangle=140)
    plt.axis('equal')
